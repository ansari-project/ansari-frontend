import { logo, messageLoader, user, warningCircle } from 'assets'
...
const Message: React.FC<MessageProps> = ({ type, content, error, loading }) => {
  return (
    <>
      <div className={`${type === 'receiver' ? 'bg-white' : 'bg-user-message-color'} md:gap-6  m-auto w-full min-h-[50px] p-4`}>
          ...
          {error && (
            <ErrorMessage>
              <Trans i18nKey='backendErrorMessage'>
                {' '}
                <a href='mailto:feedback@ansari.chat' className='text-green-bold hover:text-green-bold underline font-normal'>
                  feedback@ansari.chat
                </a>
              </Trans>
            </ErrorMessage>
          )}
          {loading && <img src={messageLoader} alt='MessageLoader image' />}
          <div
